...
def get_minimum_painting_cost(cost_matrix):
    ...
    for i in range(num_houses):
        ...
        for j in range(num_colors):
            ...
            if j==prev_house_min_index:
                sumhousei[j]= cost_matrix[i][j] + prev_house_second_min
            else:
                sumhousei[j]= cost_matrix[i][j] + prev_house_min

            if curr_house_min > sumhousei[j]:
                curr_house_second_min=curr_house_min
                curr_house_min = sumhousei[j]
                curr_house_min_index = j
            elif curr_house_second_min > sumhousei[j]:
                curr_house_second_min = sumhousei[j]

        prev_house_min = curr_house_min
        prev_house_second_min = curr_house_second_min
        prev_house_min_index = curr_house_min_index        

    return min(sumhousei)  
